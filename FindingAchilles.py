...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print(colored("Usage: python script.py <path_to_json_file>", "red"))
        sys.exit(1)
    ...
    try:
        ...
        print(colored(f"Error: File '{json_file}' not found.", "red"))
        sys.exit(1)
    except json.JSONDecodeError:
        print(colored("Error: Failed to decode JSON file.", "red"))
        sys.exit(1)
    ...
    for item in components:
        ...
        if not component or not version:
            print(colored("Error: JSON entry missing 'name' or 'version'. Skipping entry.", "red"))
            continue

        # Fetch and display CPEs
        cpe_strings = find_cpes(component, version)
        if cpe_strings:
            print(colored(f"CPEs Found for {component} {version}:", "green"))
